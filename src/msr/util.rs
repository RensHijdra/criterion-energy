...
fn read_single_core_msr_file(msr: &MSR, cpu: usize) -> Result<u64, String> {
    ...
    let file = match OpenOptions::new().read(true).write(false).open(&msr_file_name) {
        Ok(file) => file,
        Err(err) => return Err(String::from(format!("criterion-energy: could not read file {msr_file_name}: {err}"))),
    };

    let mut buf: [u8; 8] = [0, 0, 0, 0, 0, 0, 0, 0];

    match file.read_at(&mut buf, msr.numeric.clone()) {
        Ok(size) => { if size != 8 { return Err(String::from("criterion-energy: read wrong amount of data")); } }
        Err(err) => return Err(String::from(format!("criterion-energy: CPU {cpu} cannot read MSR {}: {err}", msr.string)))
    }
